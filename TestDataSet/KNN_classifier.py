...
"""
****************************************************************************************************
****    Questo script python testa il classificatore KNeighbors sui set di dati MNIST/F-MNIST   ****
...
prDir = f"KNN/Test_{datasetType[choice]}_KNN_{time.strftime('%Y_%m_%d_%H_%M_%S', time.localtime())}"
...
k = 15  # Numero di vicini -> influisce sull'accuratezza
ind_w = 0  # Indice array w
ind_m = 0  # Indice array m

w = ['uniform', 'distance']  # Weights -> influisce sul tempo di addestramento
m = ['minkowski', 'euclidean', 'manhattan']  # Metric -> influisce sul tempo di addestramento
...
writeAppend(filename=file,
            text=f"\n- Test del {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} {datasetType[choice]} con KNN "
                 f"(k: {k}, w: {w[ind_w]}, m: {m[ind_m]}):\n")
writeAppend(filename=file,
            text=f"\t+ Tempo caricamento dataset: {round((end - start), 3)} sec")


# Creazione del modello
knn = KNeighborsClassifier(n_neighbors=k, metric=m[ind_m], weights=w[ind_w])
...
printConfMatrix(namDir=f"{prDir}/{subDir1}",
                namImg="CM_train",
                namGraf=f"Train-set {datasetType[choice]} con K-Nearest Neighbor (k: {k}, m: {m[ind_m]}, w:{w[ind_w]})",
                target=Y_train,
                predict=Y_predict_train)

printConfMatrix(namDir=f"{prDir}/{subDir1}",
                namImg="CM_test",
                namGraf=f"Test-set {datasetType[choice]} con K-Nearest Neighbor (k: {k}, m: {m[ind_m]}, w:{w[ind_w]})",
                target=Y_test,
