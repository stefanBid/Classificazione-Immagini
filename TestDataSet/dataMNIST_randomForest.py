...
# Setto il numero di alberi nella foresta e la profondità
num_tree = 25
depth = 17

dotFile = open("Test_History.txt", "a")
dotFile.write(f"\n- Test del {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} MNIST con Random Forest({num_tree}/"
              f"{depth}):\n")
dotFile.write(f"\t+ Tempo caricamento dataset: {round((end - start), 3)} sec\n")
forest = RandomForestClassifier(n_estimators=num_tree, max_depth=depth, random_state=False)
...
Y_proba_test = forest.predict_proba(X_test)
Y_predict_train = forest.predict(X_train)
Y_proba_train = forest.predict_proba(X_train)
end = time.time()  # tempo fine processo
...
name = f"Test_MNIST_RF_{time.strftime('%Y_%m_%d_%H_%M_%S', time.localtime())}"
...
printConfMatrix(namDir=f"{name}/{name1}",
                namImg="CM_train",
                namGraf=f"train-set MNIST con Random Forest({num_tree}/{depth})",
                target=Y_train,
                predict=Y_predict_train)

printConfMatrix(namDir=f"{name}/{name1}",
                namImg="CM_test",
                namGraf=f"test-set MNIST con  Random Forest({num_tree}/{depth})",
                target=Y_test,
