...
"""
****************************************************************************************************
****    Questo script python testa il classificatore KNeighbors al variare degli Iper_parametri ****
...
prDir = f"KNN/Hyperparameter_Tuning_{datasetType[choice]}_KNN_{time.strftime('%Y_%m_%d_%H_%M_%S', time.localtime())}"
...
# Numero totale dei vicini su cui effettuare il test
k = 21
...
# Calcolo dell' accuratezza in base al numero di vicini
train_acc = np.zeros(k)
test_acc = np.zeros(k)


logging.info("INIZIO SEQUENZA DI ADDESTRAMENTO PER VICINI:")
...
for i in range(1, k+1):
    ...
    logging.info(f"FINE ADDESTRAMENTO DEL MODELLO CON {i} VICINI!")
...
printAccGraph(namDir=f"{prDir}/{subDir1}",
              ...
              rang=k,
              titGraf="numero vicini",
              n_x_label='Numero vicini')
