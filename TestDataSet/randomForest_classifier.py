...
"""
****************************************************************************************************
****    Questo script python testa il classificatore Foresta Randomica                          ****
...
prDir = f"RF/Test_{datasetType[choice]}_RF_{time.strftime('%Y_%m_%d_%H_%M_%S', time.localtime())}"
# Sub directory per il test
subDir1 = "Confusion_Matrix"
subDir2 = "Erroneus_Classifications"

# Creo le directory per il test
...
writeAppend(filename=file,
            text=f"\n- Test del {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} {datasetType[choice]} con "
                 f"Random Forest (profondità: {k}, c: {c[ind_c]}, mss: {mss[ind_mss]}, ne: {ne[ind_ne]}):\n")
writeAppend(filename=file,
...
printConfMatrix(namDir=f"{prDir}/{subDir1}",
                namImg="CM_train",
                namGraf=f"train-set {datasetType[choice]} con Random Forest "
                        f"(p: {k}, c: {c[ind_c]}, mss: {mss[ind_mss]}, ne: {ne[ind_ne]})",
                target=Y_train,
                predict=Y_predict_train)

printConfMatrix(namDir=f"{prDir}/{subDir1}",
                namImg="CM_test",
                namGraf=f"test-set {datasetType[choice]} con Random Forest "
                        f"(p: {k}, c: {c[ind_c]}, mss: {mss[ind_mss]}, ne: {ne[ind_ne]})",
                target=Y_test,
