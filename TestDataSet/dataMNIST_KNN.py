...
"""
********************************************************************************************
****    Questo script python testa il classificatore KNeighbors sul set di dati MNIST   ****
...
k = 15  # Indica il numero di vicini con cui addestrare la rete

dotFile = open("Test_History.txt", "a")
if k == 0:  # Caso base
    dotFile.write(f"\n- Test del {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} MNIST con KNN:\n")
    ...
    dotFile.write(f"\n- Test del {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} MNIST con KNN ({k}):\n")
    dotFile.write(f"\t+ Tempo caricamento dataset: {round((end - start), 3)} sec\n")
...
name = f"Test_MNIST_KNN_{time.strftime('%Y_%m_%d_%H_%M_%S', time.localtime())}"
...
if k == 0:
    printConfMatrix(namDir=f"{name}/{name1}",
                    namImg="CM_train",
                    namGraf="train-set MNIST con K-Nearest Neighbor",
                    target=Y_train,
                    predict=Y_predict_train)

    printConfMatrix(namDir=f"{name}/{name1}",
                    namImg="CM_test",
                    namGraf="test-set MNIST con K-Nearest Neighbor",
                    target=Y_test,
                    predict=Y_predict_test)
else:
    printConfMatrix(namDir=f"{name}/{name1}",
                    namImg="CM_train",
                    namGraf=f"train-set MNIST con {k}-Nearest Neighbor",
                    target=Y_train,
                    predict=Y_predict_train)

    printConfMatrix(namDir=f"{name}/{name1}",
                    namImg="CM_test",
                    namGraf=f"test-set MNIST con {k}-Nearest Neighbor",
                    target=Y_test,
                    predict=Y_predict_test)
